...
- **Automatic version capture, debounced** - a change to an already-tracked file becomes a new version the next time anything reads state (`docmanager status`, opening the UI). No live filesystem watcher in this version - reconciliation is triggered by a read, not continuous. A burst of rapid saves a few seconds apart (an AI or human mid-edit) doesn't each become its own version - each quick save updates the same in-progress version until things go quiet for the configured debounce window (`docmanager settings set --version-debounce <seconds>`, 5-60s, default 20s), then the next change becomes a real, permanent version. The very first version captured after tracking a document is never debounced.
...
- **Diff, revert, delete a version** - `docmanager families diff <id> <hashA> <hashB>` (a line diff on normalized content, so whitespace-only differences never show up as fake changes), `docmanager families revert <id> <hash> [--sync-file]` (moves the current version back; never edits the real file on disk unless `--sync-file` is given, which also overwrites the tracked file with that version's content), `docmanager families delete-version <id> <hash>` (permanently discards one version's record, healing the history around it - refuses on a document's only remaining version, and refuses if a live locally-mapped file still holds that exact content, since the next reconcile would just bring it right back - if you just reverted away from the version you're trying to delete, that's exactly this case: revert with `--sync-file` first, so the file no longer holds it).
...
- **Cross-document links** - not to be confused with `docmanager link` above (that declares one document *is a newer version of* another; this is about literal `<a href>` navigation between separately tracked documents). Tracking a document also tracks whatever other HTML documents it links to, following the whole reachable cluster within the folder it was tracked from - a link between two tracked documents resolves to whichever version is current *right now*, not whatever was current when the link was captured. A link added later (an edit, picked up on the next reconcile) works the same way, no extra step needed. Clicking a link inside the reading pane opens that document properly - its own title, tags, and version timeline, not just a swapped iframe - with a Back button to return to wherever you came from. A document opened via "Open in new tab" gets the same resolved links, navigating that same tab when clicked, since a plain browser tab is meant to be a fully equivalent way to read a document, not a stripped-down fallback.
- **Per-version highlights, cascading forward** - select text and mark it in one of four colors, directly, no command needed - whether you're reading inside docmanager's own pane or in a document opened via "Open in new tab." Capturing a new version automatically carries a version's highlights forward onto it, re-anchored to wherever that same text now sits - editing elsewhere in the document doesn't lose them. A highlight whose exact text was itself edited or removed isn't guessed at; it's dropped rather than risk marking the wrong words. Hover an existing highlight to remove it. Highlights sync as part of the document's own version history, the same as everything else docmanager tracks.
...
| `docmanager families revert <id> <hash> [--sync-file]` | Make an older version current again (history only, real file untouched unless `--sync-file`) |
...
| `docmanager settings get` / `set --snapshot-remote <url>` / `set --snapshot-remote-token <token>` / `set --version-debounce <seconds>` | Read or write settings |
