...
- **Per-version highlights** - select text and mark it in one of four colors, directly, no command needed - whether you're reading inside docmanager's own pane or in a document opened via "Open in new tab," both give the identical experience. A highlight belongs to the exact version it was made on: switch to a newer version of the same document and it's gone, by design - re-highlight there if you want it to carry forward. Hover an existing highlight to remove it. Highlights sync as part of the document's own version history, the same as everything else docmanager tracks.
