...
A third semantics note for this same area: revert's disk-untouched guarantee and deleteVersion's VERSION_STILL_LIVE guard are BOTH correct in isolation but can collide - reverting to an older version leaves the real file still holding the content of whatever was head before the revert, which is exactly the condition VERSION_STILL_LIVE exists to block a delete on. Don't "fix" this by weakening either guarantee. The resolution pattern: add a narrow, explicit, opt-in bridge (revertToVersion's `syncFile` option) that changes the state ON REQUEST so the existing guard's own precondition (the file no longer holds the content in question) becomes true - never touch the guard, never touch the default behavior of the thing it's guarding against.
...
A newly created highlight's <mark> is wrapped optimistically, client-side, with a placeholder id ('pending-' + Date.now()) before the create request resolves - nothing replaced it with the real server-issued id until a full reload, so removing a highlight moments after creating it (no reload in between) silently no-oped: the DELETE targeted an id the server never issued. Fixed by reassigning every <mark> sharing the placeholder id to the real one the instant it's known: standalone reads it straight from its own fetch() response; embedded (which relays the actual POST through app.js via postMessage, since the sandboxed iframe has no origin of its own to call the API directly) gets a docmanager-highlight-created-ack message echoed back from the parent page once its own API call resolves. General lesson for any future optimistic-client-side-id pattern in this codebase: a placeholder id needs an explicit reconciliation step back to the real one, in every code path that can produce the placeholder, not just the happy path where a reload happens to occur first.
