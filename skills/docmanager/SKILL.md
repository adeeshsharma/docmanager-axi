...
- `docmanager families move <id>... --to-folder <folderId>` / `--unfile` - move one or more documents into a folder, or back out to unfiled/root. Batch-capable like `track`/`untrack`, one bad id never aborts the rest.
- `docmanager folders create <name> [--parent <id>]` / `docmanager folders list` / `docmanager folders rename <id> <newName>` / `docmanager folders move <id> [--parent <id>]` / `docmanager folders delete <id>` - create, list (as a tree), rename, reparent, and delete folders, independent of any document's synthetic path. Delete refuses unless the folder is already empty.
...
- `docmanager status` - reconciles every tracked file against disk and reports current state, including anything just captured as a new version. Rapid saves a few seconds apart (an agent making several quick edits, a human mid-sentence) are debounced: each one updates the same in-progress version rather than each becoming its own permanent entry, until the configured quiet period passes. The very first version captured after tracking a document is never debounced - only the second version onward.
...
- `docmanager settings get` / `set --snapshot-remote <url>` / `set --snapshot-remote-token <token>` / `set --version-debounce <seconds>` - read or set the git remote, an access token for an HTTPS remote on a machine with no credential helper configured, and the version-debounce quiet period (one of 5/10/15/20/30/45/60, default 20 - see `docmanager status` above). `get` never returns the token's real value, only whether one is saved.
